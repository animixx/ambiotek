...
class RegistroController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $inicio = $em->getRepository('Eye3CaminosBundle:Aditivo')->InicioDatos();
        $final = $em->getRepository('Eye3CaminosBundle:Aditivo')->FinalDatos();
	...
	/**
     * 
     * @Route("/excliq/{ano}", requirements={"ano" = "(20)[0-9]{2}"}, name="genera_excel_liquidos")
	 *
     * @return response
     */
    public function excelLiqAction($ano = "2015"  ) {
			...
			$em = $this->getDoctrine()->getManager();
			$datos = $em->getRepository('Eye3CaminosBundle:Aditivo')->historial_excel($ano);
			...
			$phpExcelObject->getProperties()->setCreator("Eye3")
				...
				->setCategory("Reportes");
						...
						//titulo, tamaño letra 20, sin negrita, color blanco
						$objRichText = new PHPExcel_RichText();
						$titulo = $objRichText->createTextRun($ano." - TECK - QUEBRADA BLANCA \n");
						$titulo->getFont()->setSize(20);
						$titulo->getFont()->setBold(false);
						$titulo->getFont()->setColor(new PHPExcel_Style_Color( PHPExcel_Style_Color::COLOR_WHITE ));
						
						//subtitulo tamaño letra 13, sin negrita, color blanco
						$subtitulo = $objRichText->createTextRun("REPORTE DE CONSUMO DE AGUA Y ADITIVO EN RIEGO");
						$subtitulo->getFont()->setSize(13);
						$subtitulo->getFont()->setBold(false);
						$subtitulo->getFont()->setColor(new PHPExcel_Style_Color( PHPExcel_Style_Color::COLOR_WHITE ));

						$phpExcelObject->getActiveSheet()->getCell('D1')->setValue($objRichText);
						$phpExcelObject->getActiveSheet()->getStyle('D1')->getAlignment()->setWrapText(true);
						...
						$phpExcelObject->getActiveSheet()
								...
								->setCellValue('E2', 'Agua(m3)')
								->setCellValue('G2', 'Aditivo(m3)')
						...
						//borde grueso plomo
						$phpExcelObject->getActiveSheet()->getStyle('A3:N3')->getBorders()->getTop()
										->setBorderStyle(PHPExcel_Style_Border::BORDER_THICK);
						$phpExcelObject->getActiveSheet()->getStyle('A3:N3')->getBorders()->getTop()->getColor()->setARGB('FFD5D5D5');
				...
				foreach ($datos as $llenado)
					{	
							...
							//unir celdas
							$phpExcelObject->getActiveSheet()
								->mergeCells('B'.$aux.':D'.$aux)
								->mergeCells('E'.$aux.':F'.$aux)
								->mergeCells('G'.$aux.':H'.$aux)
								->mergeCells('I'.$aux.':K'.$aux);
								
							$phpExcelObject->getActiveSheet()->setCellValue('A'.$aux, $llenado['id'])
									->setCellValue('B'.$aux, $llenado['fecha'])
									->setCellValue('E'.$aux, $llenado['agua'])
									->setCellValue('G'.$aux, $llenado['aditivo'])
									->setCellValue('I'.$aux, $llenado['nombre']);
					...
					$phpExcelObject->getActiveSheet()->getStyle('B3:B'.$aux)
									->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
					$phpExcelObject->getActiveSheet()->getStyle('B3:B'.$aux)
									->getBorders()->getLeft()->getColor()->setARGB('FFD5D5D5');
									
					$phpExcelObject->getActiveSheet()->getStyle('E3:E'.$aux)
									->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
					$phpExcelObject->getActiveSheet()->getStyle('E3:E'.$aux)
									->getBorders()->getLeft()->getColor()->setARGB('FFD5D5D5');
									
					$phpExcelObject->getActiveSheet()->getStyle('G3:G'.$aux)
									->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
					$phpExcelObject->getActiveSheet()->getStyle('G3:G'.$aux)
									->getBorders()->getLeft()->getColor()->setARGB('FFD5D5D5');
									
					$phpExcelObject->getActiveSheet()->getStyle('I3:I'.$aux)
									->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
					$phpExcelObject->getActiveSheet()->getStyle('I3:I'.$aux)
									->getBorders()->getLeft()->getColor()->setARGB('FFD5D5D5');
									
					$phpExcelObject->getActiveSheet()->getStyle('L3:L'.$aux)
									->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
					$phpExcelObject->getActiveSheet()->getStyle('L3:L'.$aux)
									->getBorders()->getLeft()->getColor()->setARGB('FFD5D5D5');
	   
					$phpExcelObject->getActiveSheet()->getStyle('O1:O'.$aux)
									->getBorders()->getLeft()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
					$phpExcelObject->getActiveSheet()->getStyle('O1:O'.$aux)
									->getBorders()->getLeft()->getColor()->setARGB('FFD5D5D5');

					$phpExcelObject->getActiveSheet()->getStyle('A2:N'.$aux)->getAlignment()
								   ->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER)
								   ->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
					...
					if (count($datos)>0)
						$phpExcelObject->getActiveSheet()->setTitle( 'Aditivos '.$ano );
					else
						$phpExcelObject->getActiveSheet()->setTitle('Sin Mediciones');
			...
			// adding headers
			$response->headers->set('Content-Type', 'text/vnd.ms-excel; charset=utf-8');
			$response->headers->set('Content-Disposition', 'attachment;filename=Liquidos-'.$ano.'-ambiotek.xlsx');
			$response->headers->set('Pragma', 'public');
			$response->headers->set('Cache-Control', 'maxage=1');
